...
SerialMain serial(Universal_Device); 
...
void Robot_ctrl_Send(const robot_msg::Robot_ctrl::ConstPtr& Robot_ctrl_t){
    ...
    /* Test */
    ROS_INFO("L: Pitch:%.2f Yaw: %.2f Fire:%.2f Mode: %.2f",
...
int main(int argc, char *argv[]){
    ...
    // 初始化 ROS 节点
    ros::init(argc,argv,"Robot_ctrl_L_sub");

    // 创建 ROS 句柄
    ros::NodeHandle nh;

    // 创建订阅对象
    ros::Subscriber sub = nh.subscribe<robot_msg::Robot_ctrl>("Robot_ctrl_L_data",1,Robot_ctrl_Send);   

    ROS_INFO("-- Robot_ctrl_L_sub Start--");
