...
//        }
        ...
        float max_score = 0.0;
        int class_id = -1;
        for (int j = 0; j < this->detectRange; ++j) {
...
//        cout << "snallOK:" << small_wh_ratio_ok << "  bigOK:" << big_wh_ratio_ok << endl;
        ...
        if (final_grade > grade_standard)
        {
            ...
            if (candidateArmor.class_id < 18) {
                ...
                cv::Scalar color = CLASS_COLORS[armor.id % CLASS_COLORS.size()];
                int enemyColor = color[2] > color[0] ? RED : BLUE;    // 1:红(RED) 2:蓝(BLUE)
                if(enemyColor != enemy_color) continue;
            }
